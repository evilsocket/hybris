...
HYBRIS_DEFINE_FUNCTION(hbinary){
    if( data->size() < 1 ){
		hyb_syntax_error( "function 'binary' requires at least 1 parameter (called with %d)", data->size() );
	}
	...
	for( i = 0; i < data->size(); ++i ){
        hyb_type_assert( data->at(i), H_OT_INT, H_OT_CHAR );
...
void do_simple_packing( vector<unsigned char>& stream, Object *o, int size ){
	...
	if( size > o->size ){
		hyb_syntax_error( "could not pack more bytes than the object owns (trying to pack type '%s' of %d bytes to %d bytes)", Object::type_name(o), o->size, size );
	}
	switch( o->type ){
		case H_OT_INT    :
			for( i = 0; i < size; ++i ){
		...
		default:
			hyb_syntax_error( "function 'pack' does not support nested structured types" );
	}
}

HYBRIS_DEFINE_FUNCTION(hpack){
	if( data->size() < 2 ){
		hyb_syntax_error( "function 'pack' requires at least 2 parameter (called with %d)", data->size() );
	}
	...
	switch( o->type ){
		...
		case H_OT_ARRAY  :
			if( (data->size() - 1) != o->value.m_array.size() ){
				hyb_syntax_error( "not enough parameters to pack an array of %d elements (given %d)", o->value.m_array.size(), data->size() );
			}
			for( i = 1, j = 0; i < data->size(); ++i, ++j ){
				hyb_type_assert( data->at(i), H_OT_INT );
				size = data->at(i)->value.m_integer;
				do_simple_packing( stream, o->value.m_array[j], size );
			}
		break;
		case H_OT_STRUCT :
			if( (data->size() - 1) != o->value.m_struct.size() ){
				hyb_syntax_error( "not enough parameters to pack a structure with %d attributes (given %d)", o->value.m_struct.size(), data->size() );
			}
			for( i = 1, j = 0; i < data->size(); ++i, ++j ){
				hyb_type_assert( data->at(i), H_OT_INT );
				size = data->at(i)->value.m_integer;
		...
		default:
			hyb_syntax_error( "unsupported %s type in pack function", Object::type_name(o) );
	}
